...
#if RETRO_USING_KOS
// ---------------------------------------------------------------------
// Dreamcast: no Theora decoder.
//
// The OGV path (videoPlaying == 1) is stubbed out — the Toei intro/ending are
// skipped rather than decoded, which is why libtheora/libogg/theoraplay leave
// the build entirely (see platforms/KallistiOS.cmake).
//
// The RSV path (videoPlaying == 2, UpdateVideoFrame below) is UNTOUCHED: RSV
// is Retro's own palettised GIF-frame format read through the engine's file
// reader, with no codec behind it, so it still works. Sonic CD uses it for the
// in-engine "video" sprites, and losing it would be a visible regression.
// ---------------------------------------------------------------------

void PlayVideoFile(char *filePath)
{
#if DC_FMV
    // ENGINE_VIDEOWAIT is used exactly as upstream intends: this returns
    // immediately and RetroEngine.cpp calls ProcessVideo() once per frame until
    // it reports finished. An earlier version of this port blocked here and
    // owned the playback loop, which meant every fault inside the decoder froze
    // the console instead of dropping the cutscene. See DCVideo.cpp.
    if (DC_VideoStart(filePath)) {
        videoPlaying    = 1;
        Engine.gameMode = ENGINE_VIDEOWAIT;
        return;
    }
    PrintLog("Video '%s' unavailable; skipping", filePath);
#else
    PrintLog("Skipping video '%s' (built without DC_FMV)", filePath);
#endif
    // No decoder or no file: do NOT enter the video wait state, or the engine
    // sits there forever waiting for something that will never finish.
    videoPlaying = 0;
}
...
#if DC_FMV
    if (videoPlaying == 1) {
        // Any press skips, read through the engine's own input so it behaves
        // like every other "press anything" prompt in the game.
        CheckKeyPress(&keyPress, 0xFF);

        if (DC_VideoStep(anyPress != 0)) {
            StopVideoPlayback();
            return 1;
        }
        return 0;
...
#endif
    return videoPlaying == 1 ? 1 /* "finished" */ : 0;
}
...
void SetupVideoBuffer(int width, int height) { (void)width, (void)height; }
...
// RSV playback — codec-free, compiled on every platform including Dreamcast.
