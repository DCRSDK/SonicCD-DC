...
#if RETRO_USING_KOS
// Map SDL's high-resolution timer API onto KOS's nanosecond timer, so
// RetroEngine::Run's frame pacing works unchanged.
#define SDL_GetPerformanceFrequency DC_GetTicksPerSecond
#define SDL_GetPerformanceCounter   DC_GetTicks
#endif
...
#if RETRO_USING_KOS
    // Dreamcast: Y (or L+R+Start) opens the RSDK dev menu when DevMenu=true in
    // settings.ini. Mirrors the desktop SDLK_ESCAPE handler above. The edge is
    // latched in DCInput.cpp because RSDKv3's input enum has no spare slot.
    if (Engine.devMenu && DC_DevMenuEdge()) {
        if (Engine.gameMode != ENGINE_DEVMENU && Engine.gameMode != ENGINE_INITDEVMENU) {
            Engine.gameMode = ENGINE_INITDEVMENU;
        }
    }
#endif
...
void RetroEngine::Init()
{
...
#if RETRO_USING_KOS
    DC_HeapReport("pre-settings");
    DC_LOG("[DC] init: reading settings\n");
#endif
    InitUserdata();
#if RETRO_USING_KOS
    DC_LOG("[DC] init: settings ok, opening data pack\n");
    DC_Probe(2);
#endif
#if RETRO_USE_MOD_LOADER
    InitMods();
    ...
    CheckRSDKFile(dest);
#if RETRO_USING_KOS
    DC_LOG("[DC] init: data pack '%s' -> usingDataFile=%d\n", dest, Engine.usingDataFile);
    DC_Probe(3);
#endif
    ...
    running  = false;
#if RETRO_USING_KOS
    DC_LOG("[DC] init: loading GameConfig\n");
#endif
    if (LoadGameConfig("Data/Game/GameConfig.bin")) {
#if RETRO_USING_KOS
        DC_LOG("[DC] init: GameConfig ok ('%s')\n", gameWindowText);
        DC_Probe(4);
        DC_HeapReport("post-gameconfig");
#endif
        if (InitRenderDevice()) {
#if RETRO_USING_KOS
            DC_Probe(5);
...
#if RETRO_USING_KOS
                DC_Probe(6);
                DC_LOG("[DC] init: audio ok, loading first stage\n");
#endif
                InitFirstStage();
                ClearScriptData();
                initialised = true;
    ...
    if (!running) {
#if RETRO_USING_KOS
        // BASE_PATH is the GD-ROM (or the dcload host dir) — read-only either
        // way, so writing usage.txt would just fail. Log it instead; on a
        // Dreamcast the serial console is the only place a user could read it.
        DC_LOG("[DC] FATAL: engine failed to start. Check that '%s' contains %s or a Data/ folder.\n", BASE_PATH, Engine.dataFile);
        return;
#endif
        char rootDir[0x80];
...
#if RETRO_USING_KOS
                thd_pass(); // yield to the audio thread instead of busy-spinning
#endif
                continue;
            }
            prevTicks = curTicks;
