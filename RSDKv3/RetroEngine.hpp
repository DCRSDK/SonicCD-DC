...
#define RETRO_DC    (10) // SEGA Dreamcast (KallistiOS)
...
#define RETRO_PLATFORM (RETRO_DC)
...
#elif RETRO_PLATFORM == RETRO_DC
// Native Dreamcast video mode is 320x240. Widescreen/hi-res scaling has no
// meaning here and the software renderer's cost is linear in SCREEN_XSIZE,
// so this is both the correct and the fast choice.
// BASE_PATH comes from platforms/KallistiOS.cmake ("/cd/" for a disc image,
// "/pc/" for dcload) — it is used in literal-concatenation form in
// Userdata.cpp and Debug.cpp, so it must stay a plain string literal.
#ifndef BASE_PATH
#define BASE_PATH            "/cd/"
#endif
...
#if RETRO_PLATFORM == RETRO_DC
// KallistiOS backend: no SDL at all. These are #undef'd rather than merely
// left unset because CMakeLists.txt unconditionally passes RETRO_USE_SDL2=1.
#undef RETRO_USE_SDL2
#undef RETRO_USE_SDL1
#define RETRO_USING_KOS (1)
#else
#define RETRO_USING_KOS (0)
...
#if RETRO_PLATFORM == RETRO_DC
#define RETRO_USE_HAPTICS (0) // no rumble path yet (the Puru Puru pack is a maple device, not SDL haptics)
#else
...
#elif RETRO_PLATFORM == RETRO_DC
// Sonic CD's scripts only know the original 7 platforms; present as Windows so
// the standard (non-mobile) HUD/control paths are taken.
#define RETRO_GAMEPLATFORMID (RETRO_WIN)
#else
...
// NO VORBIS OR THEORA HEADERS HERE, DELIBERATELY.
//
// The Dreamcast build decodes neither. Music is raw PCM streamed by
// DC_MusicOpen and sfx are PCM, both from loose files that keep their
// original .ogg/.wav names so GameConfig.bin and the StageConfigs still
// resolve them. Video.cpp is dropped from the build entirely
// (platforms/KallistiOS.cmake removes it and theoraplay.c from RETRO_FILES),
// so the Toei intro/ending are skipped rather than decoded.
//
// Everything that referenced the vorbis API is now behind `#if !RETRO_USING_KOS`:
// StreamInfo::vorbisFile (Audio.hpp), the read/seek/tell/close callbacks and
// LoadMusic's tail (Audio.cpp). If you reinstate this include you must unwind
// those too, or the linker will start asking for ov_* symbols again.

// SDL-style fixed-width types used throughout the engine (Audio.cpp mixes in
// Sint16/Sint32, Reader.cpp and Drawing.cpp use Uint8/Uint32).
...
#if RETRO_USING_KOS
// Umbrella include so every translation unit can see the DC_* backend entry
// points without each one needing its own #if block. Deliberately BEFORE the
// engine headers: Audio.hpp's inline FreeMusInfo() calls DC_MusicClose(), and
// Drawing.cpp/Userdata.cpp call other DC_* entry points. Safe to sit here — the
// DC headers declare functions only, and the types they use (byte, uint, sbyte,
// Sint16) are all defined above.
#include "Dreamcast/DCCommon.hpp"
#include "Dreamcast/DCSave.hpp"
#endif
