...
#if RETRO_USING_KOS
// 6 MB of the Dreamcast's 16 MB would go to this array and gfxPolyList/Index if
// they were static, and the hardware path they serve is unreachable while
// renderType is pinned to RENDER_SW (InitUserdata forces it there on DC).
// Declared as a pointer instead and allocated only if a HW renderer is ever
// switched on, so a software build pays nothing. Every writer of texBuffer
// sits behind `renderType == RENDER_HW`, so NULL is never dereferenced.
ushort *texBuffer = NULL;
#else
...
#if RETRO_USING_KOS
    // The whole SDL/OpenGL body below is unreachable on Dreamcast; DCGraphics.cpp
    // owns video-mode setup and the framebuffer, and presents it in DC_FlipScreen.
    (void)gameTitle;
    return DC_InitRenderDevice();
#endif
...
#if RETRO_USING_KOS
    // The engine has already software-rendered this frame into
    // Engine.frameBuffer; DC_FlipScreen presents it. The dim/HQ/scaling logic
    // below is all desktop-window behaviour with no Dreamcast equivalent.
    DC_FlipScreen();
    return;
#endif
...
{
#if RETRO_USING_KOS
    // Always fullscreen; there is no window to resize and no desktop mode to
    // query. The whole body has to be #if'd out rather than early-returned:
    // SDL_DisplayMode / SDL_GetDesktopDisplayMode / Engine.renderer below are
    // unguarded, so they still have to compile if left in.
    (void)fs;
}
#else
    if (fs) {
...
void UpdateHardwareTextures()
{
    ...
    // ONE pass, not six.
    //
    // The loop below rebuilds the whole atlas once per palette bank because each
    // bank needs its own baked-colour texture. With indices there is nothing
    // bank-specific left in the data, so the other five passes would produce
    // byte-identical output. Skipping them is where five sixths of the VRAM and
    // five sixths of the build time go.
    SetActivePalette(0, 0, SCREEN_YSIZE);
    UpdateTextureBufferWithTiles();
    UpdateTextureBufferWithSortedSprites();

    // The upload belongs here rather than at the call sites: this function is
    // the ONLY thing that refills the staging buffer, and it has three callers
    // (stage load, dev menu init, error screen init). Doing it anywhere else
    // means one of them silently draws from a stale atlas.
    DC_AtlasUpload();
...
// Where the three atlas writers put a pixel.
//
// Everywhere except the Dreamcast this is exactly what the code did before:
// look the palette index up in the active bank and store the colour, with index
// 0 meaning transparent. Behaviour is unchanged.
//
// On the Dreamcast we store the INDEX instead, into an 8-bit atlas. That is the
// whole 12MB -> 1MB change: the six RGBA5551 atlases RSDKv3 builds differ ONLY
// in the palette lookup, so dropping the lookup makes the data bank-independent
// and one atlas replaces all six. Index 0 stays transparent because the PVR
// palette's entry 0 is set to zero alpha (see DC_AtlasUploadPalettes).
#if RETRO_USING_KOS && DC_HW_RENDER
#define ATLAS_PUT(dst, src) (dcAtlasIndices ? (void)(dcAtlasIndices[dst] = (byte)(src)) : (void)0)
#else
#define ATLAS_PUT(dst, src) (texBuffer[dst] = (src) > 0 ? fullPalette[texPaletteNum][src] : 0)
#endif
            ...
            for (int w = 0; w < 512; w += 16) {
                ...
                for (int y = 0; y < TILE_SIZE; y++) {
                    for (int x = 0; x < TILE_SIZE; x++) {
                        ATLAS_PUT(bufPos, tilesetGFXData[dataPos]);
                        bufPos++;
                        dataPos++;
                    }
...
#if RETRO_USING_KOS && DC_HW_RENDER
    // The block below stamps an opaque WHITE 16x16 square over tile 0, which the
    // hardware path then samples for every "untextured" quad — ClearScreen,
    // DrawRectangle (fades) and DrawFace all point their four vertices at one
    // texel inside it. An indexed atlas cannot hold that: white is a colour, and
    // no palette index is guaranteed to be white in every bank.
    //
    // So it is skipped here, and those quads are recognised by their four
    // identical UVs and drawn with a flat-colour PVR context instead
    // (dcQuadIsFlat / dcSubmitQuads in DCGraphics.cpp) — which is what the
    // engine meant in the first place.
    //
    // Skipping it is also what stops this writing through a null pointer:
    // texBuffer is a NULL ushort* on this platform (it is 2MB of the DC's 16MB
    // and the indexed atlas replaces it), and unlike every other texBuffer
    // access this one is not behind ATLAS_PUT.
    return;
#endif
        ...
        if (sortedSurface->texStartY + sortedSurface->height <= HW_TEXTURE_SIZE) {
            ...
            for (int h = 0; h < sortedSurface->height; h++) {
                for (int w = 0; w < sortedSurface->width; w++) {
                    ATLAS_PUT(dataPos, graphicData[gfxPos]);
                    dataPos++;
                    gfxPos++;
                }
        ...
        if (gfxSurface[i].texStartY + gfxSurface[i].height <= HW_TEXTURE_SIZE && gfxSurface[i].texStartX > -1) {
            ...
            for (int y = 0; y < gfxSurface[i].height; y++) {
                for (int x = 0; x < gfxSurface[i].width; x++) {
                    ATLAS_PUT(texPos, graphicData[gfxPos]);
...
{
#if RETRO_USING_KOS
    // Separates "stuck in object logic" from "stuck in drawing" on the hang
    // report. Reset to DC_PHASE_STAGE at the top of every frame by the run loop,
    // so it always reflects the current frame.
    DC_PHASE(DC_PHASE_DRAW);
#endif
...
#if LOD_FLOOR_GRID > 0
// One coarse floor quad: `step` units square with its near corner at (x, z),
// textured by stretching the single 16x16 sub-tile that (x, z) lands in across
// the whole thing. At the distances this is used it reads as the same ground.
//
// The fine pass in Draw3DFloorLayer spells all four flip cases out longhand, but
// they differ only in WHICH WORLD CORNER each vertex takes — the four UV values
// are consumed in the same order every time. FLIP_X is bit 0 and FLIP_Y is bit
// 1, so that whole switch collapses into the two conditionals below.
static inline void EmitCoarseFloorQuad(int x, int z, int step, int tileIndex)
{
    const int gfx = tiles128x128.gfxDataPos[tileIndex];
    const int dir = tiles128x128.direction[tileIndex];

    const float xA = (float)((dir & FLIP_X) ? x + step : x);
    const float xB = (float)((dir & FLIP_X) ? x : x + step);
    const float zA = (float)((dir & FLIP_Y) ? z + step : z);
    const float zB = (float)((dir & FLIP_Y) ? z : z + step);

    const short u0 = (short)tileUVArray[gfx];
    const short v0 = (short)tileUVArray[gfx + 1];
    const short u1 = (short)tileUVArray[gfx + 2];
    const short v1 = (short)tileUVArray[gfx + 3];

    const float vx[4] = { xA, xB, xA, xB };
    const float vz[4] = { zA, zA, zB, zB };
    const short vu[4] = { u0, u1, u0, u1 };
    const short vv[4] = { v0, v0, v1, v1 };

    for (int k = 0; k < 4; ++k) {
        polyList3D[vertexSize3D].x        = vx[k];
        polyList3D[vertexSize3D].y        = 0.0f;
        polyList3D[vertexSize3D].z        = vz[k];
        polyList3D[vertexSize3D].u        = vu[k];
        polyList3D[vertexSize3D].v        = vv[k];
...
void Draw3DFloorLayer(int layerID)
{
    ...
    if (renderType == RENDER_SW) {
        ...
        indexSize3D += 6;
        if (hq3DFloorEnabled) {
            // HQ_FLOOR_GRID (Drawing.hpp) replaces the stock hardcoded 32, so the
            // draw distance is a build setting. The three derived constants below
            // were 0x100, 0x100 and 0x200 for a grid of 32: the origin backs off
            // half the grid's width in units (GRID * 16 / 2 = GRID * 8) so the
            // sampled square is centred on the camera, and the row stride winds
            // sinValue512 back a full grid width (GRID * 16) after each row.
            // The forward bias has to SCALE WITH THE GRID, and stock RSDKv3's
            // `>> 1` only looks like a constant because it was tuned for 32.
            //
            // The grid is a square centred on the camera, then pushed along its
            // facing by this term. sin512LookupTable peaks at 512, so `>> 1`
            // gives a 256-unit push — exactly half a 32-tile grid's width, which
            // places the near edge of the square right at the camera and every
            // tile in front of it. That is why 32 is the stock number.
            //
            // Leave the push fixed and raise the grid, and the square grows
            // backwards as fast as it grows forwards: at 64 a quarter of the
            // tiles sit behind the camera, built and transformed every frame for
            // nothing. Scaling by GRID/64 keeps the near edge pinned to the
            // camera at any size, so the whole cost buys draw distance.
            //
            // `* GRID >> 6` rather than `* GRID / 64`: an arithmetic shift
            // matches the sign behaviour of the original `>> 1` exactly, so at
            // GRID 32 this is bit-for-bit the stock expression.
#if LOD_FLOOR_GRID > 0
            // ---- the distant floor, coarse -------------------------------
            //
            // Emitted BEFORE the fine grid, deliberately. Cells that only partly
            // overlap the fine square are kept rather than skipped, because a
            // gap is a hole in the floor and an overlap is invisible — and since
            // the backend draws in submission order with depth compare set to
            // ALWAYS, going first means the fine tiles paint over the coarse
            // ones wherever both exist. The other order would smear the low
            // detail back over the good stuff.
            {
                const int hqOX = ((layer->XPos >> 16) - (HQ_FLOOR_GRID * 8) + ((sin512LookupTable[layer->angle] * HQ_FLOOR_GRID) >> 6)) >> 4 << 4;
                const int hqOZ = ((layer->ZPos >> 16) - (HQ_FLOOR_GRID * 8) + ((cos512LookupTable[layer->angle] * HQ_FLOOR_GRID) >> 6)) >> 4 << 4;
                const int hqX1 = hqOX + (HQ_FLOOR_GRID * 16);
                const int hqZ1 = hqOZ + (HQ_FLOOR_GRID * 16);

                // Same centre as the fine grid, then snapped down to a whole
                // coarse cell so the two passes can never disagree about where a
                // boundary sits. The mask needs LOD_FLOOR_STEP to be a power of
                // two (asserted in Drawing.hpp) so it also works below zero.
                const int lodOX = ((hqOX + (HQ_FLOOR_GRID * 8)) - (LOD_FLOOR_GRID * LOD_FLOOR_STEP) / 2) & ~(LOD_FLOOR_STEP - 1);
                const int lodOZ = ((hqOZ + (HQ_FLOOR_GRID * 8)) - (LOD_FLOOR_GRID * LOD_FLOOR_STEP) / 2) & ~(LOD_FLOOR_STEP - 1);

                for (int li = 0; li < LOD_FLOOR_GRID; ++li) {
                    const int lz = lodOZ + li * LOD_FLOOR_STEP;
                    if (lz < 0 || lz >= layerHeight)
                        continue;

                    for (int lj = 0; lj < LOD_FLOOR_GRID; ++lj) {
                        const int lx = lodOX + lj * LOD_FLOOR_STEP;
                        if (lx < 0 || lx >= layerWidth)
                            continue;

                        // Wholly inside the fine square: the good pass has it.
                        if (lx >= hqOX && lx + LOD_FLOOR_STEP <= hqX1 && lz >= hqOZ && lz + LOD_FLOOR_STEP <= hqZ1)
                            continue;

                        // polyList3D is sized for the worst case in Drawing.hpp,
                        // so this cannot fire — but vertexSize3D is a ushort and
                        // overrunning it would look like the floor tearing apart
                        // rather than like a bug, so it is checked anyway.
                        if (vertexSize3D + 4 > VERTEX3D_COUNT)
                            break;

                        int lodIndex = currentTileMap[(lx >> 7) + ((lz >> 7) << 8)] << 6;
                        lodIndex += ((lx & 127) >> 4) + (((lz & 127) >> 4) << 3);
                        if (tiles128x128.gfxDataPos[lodIndex] > 0)
                            EmitCoarseFloorQuad(lx, lz, LOD_FLOOR_STEP, lodIndex);
                    }
                }
            }
#endif
            sinValue512 = (layer->XPos >> 16) - (HQ_FLOOR_GRID * 8);
            sinValue512 += ((sin512LookupTable[layer->angle] * HQ_FLOOR_GRID) >> 6);
            sinValue512 = sinValue512 >> 4 << 4;

            cosValue512 = (layer->ZPos >> 16) - (HQ_FLOOR_GRID * 8);
            cosValue512 += ((cos512LookupTable[layer->angle] * HQ_FLOOR_GRID) >> 6);
            cosValue512 = cosValue512 >> 4 << 4;
            for (int i = HQ_FLOOR_GRID; i > 0; i--) {
                for (int j = HQ_FLOOR_GRID; j > 0; j--) {
                ...
                sinValue512 -= (HQ_FLOOR_GRID * 16); // one full row back; was 0x200 at grid 32
                cosValue512 += 16;
            }
        }
        else {
