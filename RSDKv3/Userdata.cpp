...
bool ReadSaveRAMData()
{
    useSGame = false;
#if RETRO_USING_KOS
    return DC_LoadSaveRAM(); // read from the VMU, not the read-only disc
#endif
    char buffer[0x180];
...
{
#if RETRO_USING_KOS
    return DC_StoreSaveRAM(); // queued for DC_FlushSaves(); see DCSave.cpp
#endif
    char buffer[0x180];
    if (!useSGame) {
#if RETRO_USE_MOD_LOADER
  ...
  //  sprintf(buffer, "%s/settings.ini", getXDGDataPath().c_str());
    ...
    if (!file) {
        ...
        if (!ini.GetBool("Window", "VSync", &Engine.vsync))
            Engine.vsync = false;
#if RETRO_USING_KOS
        // Force ON — this does NOT mean "enable vsync", it means "the engine's
        // own software frame limiter is not needed".
        //
        // RetroEngine::Run only spin-waits on the tick counter when vsync is
        // false. On Dreamcast the presentation path already blocks until the
        // vertical blank (vid_waitvbl, or pvr_wait_ready on the PVR path), so
        // leaving the software limiter on means pacing the frame TWICE: once
        // against a 16.67ms tick target and again against the actual scanout.
        // The two beat against each other and can cost a whole frame.
        Engine.vsync = true;
#endif
        ...
        if (hwRender)
            ...
            renderType = RENDER_SW;
#if RETRO_USING_KOS
        // On the Dreamcast the renderer is a BUILD-time decision, not a
        // settings.ini one: the two paths need different VRAM layouts and a
        // different pvr_init, so there is nothing sensible to do with a user's
        // preference here.
        //
        // DC_HW_RENDER=ON is the indexed-atlas PVR renderer. RSDKv3's RENDER_HW
        // path normally wants six 1024x1024 RGBA5551 atlases — 12MB against the
        // DC's 8MB of VRAM — which is why this was pinned off during phases 1
        // and 2; the atlas now stores palette INDICES instead of colours, so all
        // six collapse into one 1MB texture. See PHASE3_HW_RENDERER.md.
        //
        // OFF keeps the software presenter, and with it texBuffer (Drawing.cpp)
        // permanently NULL, which is what saves 2MB of the 16MB on this platform.
#if DC_HW_RENDER
        renderType = RENDER_HW;
#else
        if (hwRender)
            PrintLog("HardwareRenderer=true ignored: this build has no HW renderer (configure with -DDC_HW_RENDER=ON)");
        renderType = RENDER_SW;
#endif
#endif
...
{
#if RETRO_USING_KOS
    // BASE_PATH is the disc (or the dcload host dir) — settings.ini is not
    // writable. Everything this would persist (volume, render type, key
    // mappings) is either fixed on Dreamcast or comes back from the .ini on
    // the disc next boot, so there is nothing to lose by skipping it.
    return;
#endif
    IniParser ini;
...
{
#if RETRO_USING_KOS
    DC_StoreUserdata(); // queued for DC_FlushSaves(); see DCSave.cpp
    return;
#endif
    char buffer[0x200];
#if RETRO_USE_MOD_LOADER
