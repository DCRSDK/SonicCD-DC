...
// va_start/va_end. Reached indirectly via SDL.h on every other platform; the
// Dreamcast build has no SDL, so include it explicitly.
#include <stdarg.h>
...
void PrintLog(const char *msg, ...)
{
    if (engineDebugMode || Engine.consoleEnabled) {
        ...
        printf("%s\n", buffer);
#if RETRO_USING_KOS
        // printf above already reaches the serial/dc-tool console, which is the
        // only log a Dreamcast user can read. BASE_PATH is the disc, so the
        // file append below would fail on every line — and it is per-line
        // fOpen/fWrite/fClose, which on a GD-ROM build means a pointless seek
        // storm. fflush so the last message before a hang isn't lost.
        fflush(stdout);
        return;
#endif
        sprintf(buffer, "%s\n", buffer);
