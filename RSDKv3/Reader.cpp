...
char fileName[0x100];
byte fileBuffer[FILEBUFFER_SIZE];
...
{
    PrintLog("WARNING: short read on '%s': got %d of %d bytes at offset %d", fileName, got, want, at);
}
#endif
int fileSize;
int vFileSize;
...
#if RETRO_USING_KOS
    // Sound effects prefer the loose Data/SoundFX tree over the data pack.
    //
    // Without this the conversion step in BUILD_DREAMCAST.md does nothing at
    // all: Engine.usingDataFile goes true the moment Data.rsdk opens, and from
    // then on EVERY LoadFile call reads the pack — so the sfx actually playing
    // are Sonic CD's desktop originals, not the Dreamcast-sized ones sitting in
    // Data/SoundFX next to the binary. Music already sidesteps the pack (DCAudio
    // streams it with fs_open); this puts sfx on the same footing, and makes the
    // asset pipeline something you can actually test a change with.
    //
    // Falls back to the pack per-file, so a partially converted SoundFX folder
    // works: whatever is present loose wins, whatever is missing comes from the
    // pack as before.
    if (Engine.usingDataFile && !Engine.forceFolder && StrLength(filePathBuf) > 13 && !memcmp(filePathBuf, "Data/SoundFX/", 13)) {
        char loosePath[0x120];
        StrCopy(loosePath, BASE_PATH);
        StrAdd(loosePath, filePathBuf);

        FileIO *probe = fOpen(loosePath, "rb");
        if (probe) {
            fClose(probe);
            StrCopy(filePathBuf, loosePath);
            Engine.forceFolder   = true;
            Engine.usingDataFile = false;
        }
    }
...
bool ParseVirtualFileSystem(FileInfo *fileInfo)
{
    ...
    if (fileOffset == -1) {
        ...
        while (i < 1) {
            ...
            DC_SEEK_SET(cFileHandle, virtualFileOffset);
            bufferPosition = 0;
            readSize       = 0;
...
void SetFileInfo(FileInfo *fileInfo)
{
    ...
    if (Engine.usingDataFile && !Engine.forceFolder) {
        ...
        DC_SEEK_SET(cFileHandle, readPos);
        FillFileBuffer();
        ...
        virtualFileOffset = 0;
        fileSize          = fileInfo->fileSize;
        // vFileSize was NOT restored here originally. It stayed at whatever the
        // previously-open file left behind, which was harmless while
        // FillFileBuffer only looked at fileSize, but the clamp added for the
        // Dreamcast's larger buffer reads it. For a loose file the whole file
        // IS the entry, so it equals fileSize.
        vFileSize = fileInfo->fileSize;
        readPos   = fileInfo->readPos;
        DC_SEEK_SET(cFileHandle, readPos);
        FillFileBuffer();
