...
// ---------------------------------------------------------------------
// THE OPCODE TABLE IS AN ABI. DO NOT COMPILE ENTRIES OUT OF IT.
// ---------------------------------------------------------------------
// HapticEffect and Engine.HapticsEnabled used to be wrapped in
// `#if RETRO_USE_HAPTICS`. On a platform where that is 0 — which is every
// Dreamcast build, since the Puru Puru pack is a maple device and not SDL
// haptics — the entry vanished from the table while the COMPILED BYTECODE ON
// THE DISC still contained the opcode. Sonic CD's mobile scripts call it: the
// special stage UFO fires one the moment you hit it.
//
// What that cost, exactly:
//
//     int opcodeSize = functions[opcode].opcodeSize;
//
// with `opcode` one past the end of the array. The interpreter read a garbage
// operand count, consumed a garbage number of bytecode words, and from that
// point on was executing operand data as instructions. It wandered until it
// happened to cycle — which is the special-stage lockup, and it had nothing to
// do with the renderer, which is why it survived the move to the PVR.
//
// So the entries stay defined unconditionally and RETRO_USE_HAPTICS now gates
// only the BEHAVIOUR. An engine that cannot rumble still has to know how wide
// the instruction is.
...
enum ScrVariable {
    ...
    VAR_ENGINEHAPTICSENABLED, // always present: see the banner above functions[]
    VAR_MAX_CNT
};
...
enum ScrFunction {
    ...
    FUNC_LOADONLINEMENU,
    FUNC_ENGINECALLBACK,
    FUNC_HAPTICEFFECT, // always present: see the banner above functions[]
    FUNC_MAX_CNT
};
...
#if RETRO_USING_KOS
// The watchdog named the OBJECT that hung; this names the INSTRUCTION. One
// store per opcode in the interpreter's hot loop, which is a rounding error
// against the dozens of memory accesses each opcode already does, and the
// difference between "the UFO script is stuck" and "the UFO script is stuck on
// PlaySfx".
volatile unsigned short dcScriptOp  = 0;
volatile unsigned char dcScriptSub  = 0;

const char *DC_ScriptOpName()
{
    const int op = dcScriptOp;
    if (op < 0 || op >= FUNC_MAX_CNT)
        return "?";
    return functions[op].name;
}
unsigned short DC_ScriptOp() { return dcScriptOp; }
unsigned char DC_ScriptSub() { return dcScriptSub; }

// ---------------------------------------------------------------------
// Opcode budget
// ---------------------------------------------------------------------
// A script that loops forever takes the whole console with it, and the only
// thing the hang report can then say is "it was in here somewhere". This puts a
// ceiling on ONE call to ProcessScript, so a runaway loop becomes a dropped
// frame for one object instead of a lockup - and, far more usefully, it
// captures where the loop was before killing it.
//
// The number: a legitimate script call is tens to low hundreds of opcodes.
// Even the heaviest setup routine in Sonic CD does not approach five figures.
// 250000 is far beyond anything real, and at SH4 speed the budget itself burns
// roughly 30-50 ms before it fires, so a false positive would show up as one
// stutter rather than as a crash.
//
// dcScriptLoopLo/Hi are the bytecode range the interpreter was cycling over in
// the last complete window of DC_SCRIPT_WINDOW opcodes. For a runaway loop that
// window sits entirely inside the loop, so the two numbers bracket the
// offending code. Tracking min/max over the WHOLE call would instead include
// the prologue and say nothing.
#define DC_SCRIPT_OP_BUDGET (250000)
#define DC_SCRIPT_WINDOW    (2048)

volatile unsigned int dcScriptAborts    = 0;  // how many calls have been killed
volatile int dcScriptAbortPtr           = -1; // scriptCodePtr at the kill
volatile int dcScriptAbortStart         = -1; // scriptCodeStart of the killed call
volatile int dcScriptLoopLo             = 0;
volatile int dcScriptLoopHi             = 0;
volatile unsigned short dcScriptAbortOp = 0;
volatile unsigned char dcScriptAbortSub = 0;
volatile short dcScriptAbortObj         = -1;
volatile short dcScriptAbortType        = -1;

// The last DC_SCRIPT_TRAIL opcodes before the kill, oldest first. A runaway loop
// is a short cycle repeated, so this IS the loop body — read the numbers off the
// screen, look them up in the function table, and the shape of the loop is
// there without needing the bytecode at all.
//
// Cost in the hot path is one byte store and one increment. The ring is not
// volatile: nothing reads it until the abort has already happened.
#define DC_SCRIPT_TRAIL (10)
static byte dcOpRing[16];
static int dcPtrRing[16];
static uint dcOpRingPos = 0;
volatile byte dcScriptAbortTrail[DC_SCRIPT_TRAIL];
volatile int dcScriptAbortTrailPtr[DC_SCRIPT_TRAIL];

byte DC_ScriptAbortTrail(int i)
{
    if (i < 0 || i >= DC_SCRIPT_TRAIL)
        return 0;
    return dcScriptAbortTrail[i];
}
int DC_ScriptAbortTrailLen() { return DC_SCRIPT_TRAIL; }
// Where each trailed opcode was fetched from. The opcode numbers say WHAT ran;
// these say WHERE, and a backwards step between two consecutive entries is a
// jump — which is the thing that has to be identified, because a script loop
// that never ends is either a loop whose condition never goes false or a jump
// that landed somewhere it should not have, and the two look identical until
// you can see the addresses.
int DC_ScriptAbortTrailPtr(int i)
{
    if (i < 0 || i >= DC_SCRIPT_TRAIL)
        return 0;
    return dcScriptAbortTrailPtr[i];
}
...
unsigned int DC_ScriptAborts() { return dcScriptAborts; }
int DC_ScriptAbortPtr() { return dcScriptAbortPtr; }
int DC_ScriptAbortStart() { return dcScriptAbortStart; }
int DC_ScriptLoopLo() { return dcScriptLoopLo; }
int DC_ScriptLoopHi() { return dcScriptLoopHi; }
unsigned short DC_ScriptAbortOp() { return dcScriptAbortOp; }
unsigned char DC_ScriptAbortSub() { return dcScriptAbortSub; }
short DC_ScriptAbortObj() { return dcScriptAbortObj; }
short DC_ScriptAbortType() { return dcScriptAbortType; }
const char *DC_ScriptAbortOpName()
{
    const int op = dcScriptAbortOp;
    if (op < 0 || op >= FUNC_MAX_CNT)
        return "?";
    return functions[op].name;
}
...
#if RETRO_USING_KOS
    dcScriptSub = scriptSub;
#endif

    jumpTableStackPos = 0;
    functionStackPos  = 0;
#if RETRO_USING_KOS
    unsigned int dcBudget = DC_SCRIPT_OP_BUDGET;
    unsigned int dcWindow = DC_SCRIPT_WINDOW;
    int dcLo = scriptCodePtr, dcHi = scriptCodePtr;
#endif
    while (running) {
        int opcode           = scriptCode[scriptCodePtr++];
#if RETRO_USING_KOS
        dcScriptOp                 = (unsigned short)opcode;
        dcOpRing[dcOpRingPos & 15]  = (byte)opcode;
        dcPtrRing[dcOpRingPos & 15] = scriptCodePtr - 1;
        ++dcOpRingPos;

        // Rolling window: keep the extent of the last DC_SCRIPT_WINDOW opcodes,
        // publish it when the window closes, then start a fresh one. Two
        // compares and a decrement per opcode.
        if (scriptCodePtr < dcLo)
            dcLo = scriptCodePtr;
        if (scriptCodePtr > dcHi)
            dcHi = scriptCodePtr;
        if (!--dcWindow) {
            dcWindow       = DC_SCRIPT_WINDOW;
            dcScriptLoopLo = dcLo;
            dcScriptLoopHi = dcHi;
            dcLo = dcHi = scriptCodePtr;
        }

        if (!--dcBudget) {
            // Record everything about the kill BEFORE unwinding, then leave the
            // interpreter the way FUNC_END would. jumpTableStackPos and
            // functionStackPos are reset on entry to every call, so abandoning
            // them here leaks nothing into the next object.
            ++dcScriptAborts;
            dcScriptAbortPtr   = scriptCodePtr;
            dcScriptAbortStart = scriptCodeStart;
            dcScriptAbortOp    = (unsigned short)opcode;
            dcScriptAbortSub   = scriptSub;
            dcScriptAbortObj   = (short)objectLoop;
            dcScriptAbortType  = (short)((objectLoop >= 0 && objectLoop < ENTITY_COUNT) ? objectEntityList[objectLoop].type : -1);
            for (int t = 0; t < DC_SCRIPT_TRAIL; ++t) {
                const uint slot          = (dcOpRingPos - DC_SCRIPT_TRAIL + t) & 15;
                dcScriptAbortTrail[t]    = dcOpRing[slot];
                dcScriptAbortTrailPtr[t] = dcPtrRing[slot];
            }
            PrintLog("[DC] SCRIPT ABORT: obj=%d type=%d sub=%d start=%d ptr=%d op=%d(%s) loop=[%d,%d]", (int)dcScriptAbortObj,
                     (int)dcScriptAbortType, (int)scriptSub, scriptCodeStart, scriptCodePtr, opcode, functions[opcode].name,
                     dcScriptLoopLo, dcScriptLoopHi);
            break;
        }
#endif
        int opcodeSize       = functions[opcode].opcodeSize;
...
#if RETRO_USE_HAPTICS
                        scriptEng.operands[i] = Engine.hapticsEnabled;
#else
                        scriptEng.operands[i] = 0; // no rumble here, but the variable still has to exist
#endif
                        break;
                }
            }
        ...
        switch (opcode) {
            ...
            case FUNC_HAPTICEFFECT:
                // opcodeSize = 0 whether or not anything rumbles: it stops the
                // operand write-back below, and skipping that is half the point
                // of the case existing.
                opcodeSize = 0;
#if RETRO_USE_HAPTICS
        ...
        for (int i = 0; i < opcodeSize; ++i) {
            ...
            if (opcodeType == SCRIPTVAR_VAR) {
                ...
                switch (scriptCode[scriptCodePtr++]) {
                    ...
                    case VAR_ENGINEHAPTICSENABLED:
#if RETRO_USE_HAPTICS
                        Engine.hapticsEnabled = scriptEng.operands[i];
#endif
                        break;
                }
            }
